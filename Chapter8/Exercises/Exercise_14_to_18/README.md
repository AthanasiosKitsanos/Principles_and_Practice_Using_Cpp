...
`Money` has an accuracy of 1 cent at most,  
while a `Rational` number an accuracy as long as the integers allow

### EXERCISE 18
**Give an example of a calculation where a `Rational` gives a mathematically better result than `double`.**  
A `double` rounds up or down the result, while a `Rational` doesn't