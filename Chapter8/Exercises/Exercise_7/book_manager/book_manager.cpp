...
namespace library
{
    book_manager::book_manager(): c('\0')
    {
        ...
        m_book_lib.emplace_back("The Silent Observer", "Elena Marlowe", "123A", copyright_date{2018, 5, 12}, genre::fiction);
        m_book_lib.emplace_back("Shadows of Glass", "Marcus Trent", "847B", copyright_date{2019, 11, 3}, genre::periodical);
        m_book_lib.emplace_back("The Quantum Thief", "Hannu Rajaniemi", "314R", copyright_date{2011, 2, 20}, genre::fiction);
        m_book_lib.emplace_back("Dust and Code", "Nina Karras", "762K", copyright_date{2021, 7, 14}, genre::biography);
        m_book_lib.emplace_back("Axioms of Power", "Tobias Reinhardt", "901T", copyright_date{2021, 7, 14}, genre::children);
        vec_sort(m_book_lib);
    }
    ...
    void book_manager::add_new_book()
    {
        ...
        if(isbn_exists(isbn)) // added to check if the isbn already exists before adding the new book
        {
        ...
        genre gen = write_genre();
        int pos{vec_lower_bound(title)};
        m_book_lib.emplace(m_book_lib.begin() + pos, title, author, isbn, copyright_date{year, month, day}, gen);
    ...
    {
        for(const book& b: m_book_lib)
        {
            if(b.get_isbn() == isbn) return true;
        }
        return false;
    }

    genre book_manager::write_genre()
    {
        std::string gen = m_is.get_input(c);
        for(char& ch: gen) ch = std::tolower(ch);
        if(gen == "fiction") return genre::fiction;
        if(gen == "nonfiction") return genre::nonfiction;
        if(gen == "children") return genre::children;
        if(gen == "biography") return genre::biography;
        if(gen == "periodicak") return genre::periodical;
