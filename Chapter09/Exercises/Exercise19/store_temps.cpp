...
int main()
{
    ...
    for(readings rd{}; std::getline(std::cin, input);)
    {
        ...
        rd.suffix = *start;
        if(!rd.is_valid_suffix()) std::cout << "Invalid suffix. Enter 'c' or 'f'" << std::endl;
        else ofs << "Hour: " << rd.hour << std::setw(16) << std::setprecision(2) << "Temperature: " << rd.temperature << (rd.suffix == 'c' ? " Celsius" : " Fahrenheit") << '\n';
